...
object AST {
  ...
  def tokenize(ast: AST): Shifted.List1[AST] = {
    @tailrec
    def go(ast: AST, out: Stream): Shifted.List1[AST] = ast match {
    ...
    implicit def instance[T[_]](
      ...
      new UnapplyByType[T[AST]] {
        def unapply(t: AST) =
          ...
          // as the original one.
          ct.unapply(t.shape).map(_ => t.asInstanceOf[T[AST]])
      }
  }
  ...
  final case class AST(shape: ShapeOf[AST], id: Option[ID] = None)

  implicit def toAST(shape: ShapeOf[AST]): AST = AST(shape, None)
  ...
  object Invalid {
    ...
    type Unrecognized = UnrecognizedOf[AST]
    type Unexpected   = UnexpectedOf[AST]
  ...
  object Ident {
    ...
    implicit def reprForIdent: Repr[Ident] = {
      case t: BlankOf[_] => t.repr
      case t: VarOf[_]   => t.repr
      case t: ConsOf[_]  => t.repr
      case t: OprOf[_]   => t.repr
      case t: ModOf[_]   => t.repr
    }
    ...
    type InvalidSuffix = InvalidSuffixOf[AST]
  ...
  object Literal {
    ...
    object Text {
      ...
      case class UnclosedOf[T](text: TextOf[T]) extends InvalidOf[T]
...
//
  ...
  object Comment {
    ...
    val symbol                              = "#"
    def apply(lines: List[String]): Comment = CommentOf(lines)
  }
...
//  }
  ...
  object MixfixOf {
    ...
    implicit def repr[T: Repr]: Repr[MixfixOf[T]] = t => {
      ...
      val nameRepr = t.name.toList.map(Ident.reprForIdent.repr(_))
  ...
  def main() {
    ...
    val fff1 = Ident.BlankOf[AST](): Ident.BlankOf[AST]
    val fff3 = fff1: Blank
    val fff4 = fff3: AST
